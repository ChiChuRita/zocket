...
`useCall` expects a promise. If your server handler returns `void`, consider returning an “ack” value like `{ ok: true }` so the client call is awaitable and can be used with `useCall`.
...
function NewMessage() {
  const sendMessage = zocket.useMutation(
    (client, text: string) => client.chat.post({ text })
  );
